...
public class RelatedFilesFormatter implements IFormatter {
    private final static Logger LOG = LoggerFactory.getLogger(RelatedFilesFormatter.class);
    ...
    private List<QueryStructure> buildQueryList(long objectId, boolean withoutDuplicate){
		...
		QueryStructure.Builder duplicateQueryStructure = QueryStructure.newBuilder().setType(QueryType.BY_ATTR_NAME).setAttrName("duplicate").setNegate(withoutDuplicate);
